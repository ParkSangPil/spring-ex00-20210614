 ...
 */
 $(function(){
	...
	/*  시간 꾸미기  */
	function getCurrentDate(rdate) {
	
	    var week = new Array('일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일');
	    var today = new Date(rdate).getDay();
	    var yoil = week[today];

		var now = new Date(rdate);
	    ...
	    return year + "-" + month + "-" + date + " [ " + yoil + " " + hours + " : " + minutes + " : " + seconds + " ] ";
    }
	...
	$("#reply-insert-btn1").click(function() {
		...
		$.ajax({
			...
			success: function() {
				...
				/* setTimeout(function() {
					$("#alert1").alert("close");             이 부분 문제점 새 댓글을 입력시 hidden 풀리고 alert까지 닫아서 
				}, 3000);                                    그 다음 새 댓글 작성이 애매해짐 그냥 이 alert가 뜨는 장소 */
			},                                               /* 의 위치를 잘 하거나 혹은 다른 방법을 갈구 해야 할듯 */
			error: function() {
				console.log("입력 실패");
			}
		});
